...
struct DoublyLinkedList {
    ...
    void addHead(const string& element) {
        ...
        if (head) head->prev = newNode; // Если список не пуст, установить указатель на предыдущий узел у первого узла
        head = newNode; // Установка нового узла как первого
        if (!tail) tail = head; // Если список был пуст, установить новый узел как последний
    }
