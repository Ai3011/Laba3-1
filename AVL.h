...
struct QueueForAvl {
    ...
    void enqueue(AVLNode* node) {
        QueueNode* newNode = new QueueNode();  // Создаем новый узел очереди
        newNode->treeNode = node;              // Присваиваем узел дерева в новый узел очереди
        newNode->next = nullptr;               // Устанавливаем, что следующий элемент в очереди — nullptr, так как это последний элемент

        if (rear == nullptr) {                 // Если очередь пуста (нет элементов)
            front = rear = newNode;            // Устанавливаем новый узел как начало и конец очереди
        }
        else {
            rear->next = newNode;              // Присоединяем новый узел к концу текущей очереди
            rear = newNode;                    // Перемещаем указатель на конец очереди к новому узлу
        }
        count++;  // Увеличиваем счетчик элементов в очереди
    }

    // Удаление узла из очереди и возвращение его
    AVLNode* dequeue() {
        if (isEmpty()) return nullptr;         // Если очередь пуста, возвращаем nullptr

        QueueNode* temp = front;               // Сохраняем указатель на текущий первый элемент очереди
        AVLNode* result = front->treeNode;     // Сохраняем узел дерева из первого элемента очереди для возврата
        front = front->next;                   // Перемещаем указатель начала очереди на следующий элемент

        if (front == nullptr) {                // Если очередь стала пуста после удаления элемента
            rear = nullptr;                    // Обнуляем указатель на конец очереди
        }

        delete temp;                           // Удаляем предыдущий первый элемент, освобождаем память
        count--;                               // Уменьшаем счетчик элементов в очереди
        return result;                         // Возвращаем узел дерева из удаленного элемента
    }
